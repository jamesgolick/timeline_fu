...
class FiresTest < Test::Unit::TestCase
  ...
  def test_should_fire_event_with_secondary_subject
    ...
    TimelineEvent.expects(:create!).with(actor: @mat, subject: @comment, secondary_subject: @list, event_type: 'comment_created')
    @comment.save
  end
  ...
  def test_should_set_secondary_subject_to_self_when_requested
    @list = List.new(hash_for_list(author: @james));
    TimelineEvent.expects(:create!).with(actor: @james, subject: @article, event_type: 'list_created_or_updated')
    @list.save
    @comment = Comment.new(body: 'cool list!', author: @mat, list: @list)
    TimelineEvent.expects(:create!).with(actor: @james, subject: @article, event_type: 'comment_created')
    @comment.save
    TimelineEvent.expects(:create!).with(actor: @mat, subject: @list, secondary_subject: @comment, event_type: 'comment_deleted')
    @comment.destroy
  end
